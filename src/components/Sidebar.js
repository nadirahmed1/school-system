...
export default function Sidebar({ page, setPage, user, onLogout }) {
  return (
    <div style={{
      ...
      display: 'flex', flexDirection: 'column', padding: '0', boxShadow: '4px 0 20px rgba(0,0,0,0.3)'
    }}>
      {/* Logo */}
      <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.75rem', margin: '0.2rem 0 0' }}>
          {user.dugsi_magac || 'Dugsi'}
        </p>
      </div>
      ...
      <nav style={{ flex: 1, padding: '1rem 0' }}>
        {menuItems.map(item => (
          <button key={item.id} onClick={() => setPage(item.id)} style={{
            width: '100%', padding: '0.85rem 1.5rem', background: page === item.id
      ...
      <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        <button onClick={onLogout} style={{
          width: '100%', padding: '0.75rem', background: 'rgba(231,76,60,0.2)',
          border: '1px solid rgba(231,76,60,0.4)', borderRadius: '8px',
          color: '#e74c3c', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600'
        }}>
          🚪 Ka Bax
        </button>
      </div>
