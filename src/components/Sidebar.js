...
// ── Settings helper (copy-ka Settings.js ka) ──────────────
...
export default function Sidebar({ page, setPage, user, onLogout }) {
  ...
  // Marka Settings-ka la beddelo, otomaatig u cusboonaysii
  useEffect(() => {
    function onChange(e) { setSettings(e.detail) }
    window.addEventListener('school_settings_changed', onChange)
    return () => window.removeEventListener('school_settings_changed', onChange)
  }, [])

  // Magaca dugsigu: Settings → localStorage dugsi_magac → user → default
  const dugsiMagac =
    settings.schoolName ||
