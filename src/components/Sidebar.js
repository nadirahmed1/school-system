...
export default function Sidebar({ page, setPage, user, onLogout }) {
  return (
    <div style={{
      width: '240px', minHeight: '100vh', background: 'linear-gradient(180deg, #1a3a5c 0%, #0f2744 100%)',
      ...
      <div style={{ padding: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#fff', margin: '0.5rem 0 0', fontSize: '1rem', fontWeight: '700' }}>
          {localStorage.getItem('dugsi_magac') || 'Nidaamka Dugsiga'}
        </h2>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.72rem', margin: '0.2rem 0 0' }}>Maamulka Dugsiga</p>
      </div>

      <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: user.role === 'admin' ? '#e74c3c' : '#27ae60', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '700' }}>
      ...
      <nav style={{ flex: 1, padding: '1rem 0' }}>
        {menuItems.filter(item => !item.adminOnly || user.role === 'admin').map(item => (
          <button key={item.id} onClick={() => setPage(item.id)} style={{
            ...
            border: 'none', borderLeft: page === item.id ? '4px solid #3498db' : '4px solid transparent',
            color: page === item.id ? '#fff' : 'rgba(255,255,255,0.6)',
            display: 'flex', alignItems: 'center', gap: '0.75rem',
            cursor: 'pointer', fontSize: '0.9rem', fontWeight: page === item.id ? '600' : '400',
      ...
      <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        <button onClick={onLogout} style={{ width: '100%', padding: '0.75rem', background: 'rgba(231,76,60,0.2)', border: '1px solid rgba(231,76,60,0.4)', borderRadius: '8px', color: '#e74c3c', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600' }}>
          🚪 Ka Bax
        </button>
      </div>
    </div>
