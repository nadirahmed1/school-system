...
export default function Macallimiin() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      // Ku dar kaliya columns-yada jira — ha ku darin wax aan jirin
      ...
      if (form.email.trim()) saveData.email = form.email.trim().toLowerCase()
      if (form.password_hash.trim()) saveData.password_hash = form.password_hash.trim()
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '480px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {[
              ...
              { label: 'Email', key: 'email', placeholder: 'macalin@dugsi.so', type: 'email' },
              { label: editId ? 'Password Cusub (ikhtiyaari)' : 'Password (ikhtiyaari)', key: 'password_hash', placeholder: '••••••••', type: 'password' },
            ].map(f => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.2rem' }}>
        {loading ? (
          ...
          <div key={m.id} style={{ background: '#fff', borderRadius: '14px', padding: '1.5rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', borderTop: '4px solid #27ae60' }}>
            ...
            {m.email && <div style={{ fontSize: '0.85rem', color: '#5d6d7e', marginBottom: '0.4rem' }}>📧 {m.email}</div>}
