...
export default function Macallimiin() {
  ...
  const save = async () => {
    ...
    if (!form.email.trim()) return setMsg('⚠️ Email qor!')
    if (!editId && !form.password_hash.trim()) return setMsg('⚠️ Password qor!')
    setSaving(true)
    try {
      const saveData = { 
        magaca: form.magaca.trim(),
        telefoon: form.telefoon.trim(),
        maado: form.maado.trim(),
        mushaharka: Number(form.mushaharka) || 0,
        email: form.email.trim().toLowerCase(),
      }
      // Only include password if provided
      if (form.password_hash.trim()) {
        saveData.password_hash = form.password_hash
      }
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '480px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {[
              ...
              { label: editId ? 'Password Cusub (haddaad doonto)' : 'Password *', key: 'password_hash', placeholder: '••••••••', type: 'password' },
            ].map(f => (
