...
export default function Lacagta({ user }) {
  ...
  // Raadi ID-ka bishaha oo dhan (dhammaan bilaha xogta ku jira)
  ...
  // ID search filter — haddii ID la qoro, bishaha oo dhan ka raadi
  const isSearching = idSearch.trim().length > 0
  const filteredById = isSearching
    ? (filter === 'bixiyay' ? idSearchResults.filter(l => l.bixiyay)
      : filter === 'mabixin' ? idSearchResults.filter(l => !l.bixiyay)
      : idSearchResults)
    : filtered
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:'0.5rem', marginBottom:'1.2rem', flexWrap:'wrap' }}>
        {[['all','📋 Dhammaan'], ['bixiyay','✅ Bixiyay'], ['mabixin','❌ Ma Bixin']].map(([val, label]) => {
          const countAll     = isSearching ? idSearchResults.length                        : lacagta.length
          const countBixiyay = isSearching ? idSearchResults.filter(l => l.bixiyay).length : bixiyayList.length
          const countMabixin = isSearching ? idSearchResults.filter(l => !l.bixiyay).length : tiradaMaBixinDhabta
          const count = val === 'bixiyay' ? countBixiyay : val === 'mabixin' ? countMabixin : countAll
          return (
