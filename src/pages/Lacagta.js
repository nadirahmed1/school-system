...
export default function Lacagta({ user }) {
  ...
  const filtered = lacagta.filter(l => 
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          { label: 'Wadarta Bixiyay', value: `$${totalBixiyay.toLocaleString()}`, color: '#27ae60', icon: '✅' },
          { label: 'Wadarta Ma Bixin', value: `$${totalMaBixin.toLocaleString()}`, color: '#e74c3c', icon: '📋' },
          { label: 'Wadarta Guud', value: `$${(totalBixiyay + totalMaBixin).toLocaleString()}`, color: '#3498db', icon: '💰' },
        ].map((s, i) => (
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '460px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input 
                type="number" 
                value={form.xaddiga} 
                onChange={e => setForm({ ...form, xaddiga: e.target.value })} 
