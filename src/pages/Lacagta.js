...
export default function Lacagta({ user }) {
  ...
  // ── CUSUB: Search field table-ka kujira ──
  ...
  const fetchBisha = useCallback(async (bisha, sannad) => {
    setLoading(true)
    setFilter('all')
    setTableSearch('') // reset search marka bisha la beddesho
  ...
  const checkMonthlyReset = async () => {
    ...
    if (lastReset === thisMonth) return

    setResetting(true)
    try {
      const { data: allArday } = await supabase.from('ardayda').select('id')
  ...
  const save = async () => {
    ...
    const insertData = {
      ...
      bixiyay:            false,  // ── CUSUB: had iyo jeer false ku bilow, user badhanka uu ku bedeli
      taariikhda_bixinta: null,
    }
    ...
    if (error) {
      ...
      setMsg('✅ Lacag la diiwaangeliyay! Xaaladda: Ma Bixin — badhanka la isticmaali si loo beddelo.')
      setShowForm(false)
      setForm(emptyForm())
      setArdaySearch('')
  ...
  // ── HAGAAJIN: isticmaal lacagta list-ka dhabta ah, ha isticmaalin ardayda guud ──
  const tiradaBixiyay   = bixiyayList.length
  const tiradaMaBixin   = mabixinList.length

  // Filter + Search isticmaal waxba
  const baseFiltered =
    filter === 'bixiyay' ? bixiyayList :
    filter === 'mabixin' ? mabixinList :
    lacagta

  // ── CUSUB: Table search (ID ama magac) ──
  const filtered = tableSearch.trim().length === 0
    ? baseFiltered
    : baseFiltered.filter(l =>
        l.ardayda?.magaca?.toLowerCase().includes(tableSearch.toLowerCase()) ||
        l.ardayda?.arday_id_gaar?.toLowerCase().includes(tableSearch.toLowerCase())
      )
...
${lacag.bixiyay?`<div class="info-row"><span class="info-label">📆 Taariikhda Bixinta</span><span class="info-value">${taariikhBixin}</span></div>`:''}</div>
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px, 1fr))', gap:'1rem', marginBottom:'1.5rem' }}>
        {[
          ...
          },
          {
            ...
            sub:       `${tiradaMaBixin} arday`,   // ── HAGAAJIN: tirada dhabta ah
            color:     '#e74c3c',
            filterVal: 'mabixin',
          },
          {
      ...
      <div style={{ display:'flex', gap:'0.75rem', marginBottom:'1.2rem', flexWrap:'wrap', alignItems:'center' }}>
        <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
          {[['all','📋 Dhammaan'], ['bixiyay','✅ Bixiyay'], ['mabixin','❌ Ma Bixin']].map(([val, label]) => (
            <button key={val} onClick={() => { setFilter(val); setTableSearch('') }} style={{
              ...
              <span style={{
                ...
                ({val === 'bixiyay' ? tiradaBixiyay : val === 'mabixin' ? tiradaMaBixin : lacagta.length})
              </span>
            </button>
          ))}
        </div>
        ...
        <div style={{ marginLeft:'auto', position:'relative' }}>
          ...
          {tableSearch && (
            <button onClick={() => setTableSearch('')} style={{
              position:'absolute', right:'0.6rem', top:'50%', transform:'translateY(-50%)',
              background:'none', border:'none', cursor:'pointer', color:'#95a5a6', fontWeight:'700', fontSize:'0.9rem', lineHeight:1
            }}>✕</button>
          )}
        </div>
      </div>
      ...
      {tableSearch.trim().length > 0 && (
        <p style={{ margin:'-0.5rem 0 0.8rem', fontSize:'0.8rem', color:'#7f8c8d' }}>
          🔍 "<strong>{tableSearch}</strong>" — {filtered.length} arday la helay
        </p>
      )}
      ...
      {showForm && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'1rem' }}>
          <div style={{ background:'#fff', borderRadius:'16px', padding:'2rem', width:'100%', maxWidth:'460px', boxShadow:'0 25px 60px rgba(0,0,0,0.3)', maxHeight:'90vh', overflowY:'auto' }}>
            ...
            {/* ── CUSUB: Halkaan checkbox-ka waa la maqay — lacagtu otomaatig "Ma Bixin" ku bilaabmaysaa ── */}
            <div style={{ marginBottom:'1.5rem', padding:'0.75rem 1rem', background:'#fff8e1', borderRadius:'8px', border:'1px solid #f0c050', fontSize:'0.85rem', color:'#7a6000' }}>
              ℹ️ Marka la diiwaangeliyo, arday <strong>otomaatig "Ma Bixin"</strong> ayuu ku bilaabanayaa. Badhanka xaaladda riix si aad u beddeshid.
            </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:'3rem' }}>
            <p style={{ fontSize:'2rem', marginBottom:'0.5rem' }}>
              {tableSearch ? '🔍' : filter === 'bixiyay' ? '✅' : filter === 'mabixin' ? '❌' : '📋'}
            </p>
            <p style={{ color:'#95a5a6', fontWeight:'600' }}>
              {tableSearch
                ? `"${tableSearch}" — arday lama helin`
                : filter === 'bixiyay' ? 'Lacag bixiyay lama helin'
                : filter === 'mabixin' ? 'Lacag ma bixin lama helin'
                : 'Xog lama helin'
              } — {filterBisha} {filterSannad}
            </p>
          </div>
        ) : (
