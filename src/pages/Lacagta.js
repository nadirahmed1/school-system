...
export default function Lacagta({ user }) {
  ...
  const [idSearchResults, setIdSearchResults] = useState([])  // natiijada raadinta oo dhan bil
  ...
  // Raadi ID-ka bishaha oo dhan (dhammaan 12 bilood)
  const searchAllMonths = useCallback(async (query) => {
    if (!query.trim()) { setIdSearchResults([]); return }

    // Hel arday-ga ID-kiisa ku habboon
    const { data: ardayMatch } = await supabase
      .from('ardayda')
      .select('id, magaca, fasalka, arday_id_gaar')
      .ilike('arday_id_gaar', `%${query.trim()}%`)

    if (!ardayMatch || ardayMatch.length === 0) { setIdSearchResults([]); return }

    // Hel dhammaan lacagta arday-gaas (bishaha oo dhan, sannadka oo dhan)
    const ardayIds = ardayMatch.map(a => a.id)
    const { data: lacagData } = await supabase
      .from('lacagta')
      .select('*, ardayda(magaca, fasalka, arday_id_gaar, telefoon)')
      .in('arday_id', ardayIds)
      .order('sannadka', { ascending: false })

    // Ku dar dhammaan 12 bilood — haddii bil xog la'aantahay, placeholder ku dar
    const results = []
    ardayMatch.forEach(arday => {
      BISHOOYINKA.forEach(bil => {
        const found = (lacagData || []).find(
          l => l.arday_id === arday.id && l.bisha === bil && l.sannadka === filterSannad
        )
        if (found) {
          results.push(found)
        } else {
          // Placeholder — xog la'aan
          results.push({
            id: `placeholder-${arday.id}-${bil}`,
            arday_id: arday.id,
            bisha: bil,
            ...
            bixiyay: null,
            placeholder: true,
            ardayda: { magaca: arday.magaca, fasalka: arday.fasalka, arday_id_gaar: arday.arday_id_gaar }
          })
        }
      })
    })
  ...
  useEffect(() => {
    searchAllMonths(idSearch)
  }, [idSearch, searchAllMonths, filterSannad])
  ...
  // ID search filter — haddii ID la qoro, bishaha oo dhan ka raadi (bil doorasho jooji)
  const isSearching = idSearch.trim().length > 0
  const filteredById = isSearching ? idSearchResults : filtered
...
<script>window.onload=function(){window.print()}</script></body></html>`)
  ...
  return (
    <div>
        ...
        {[['all','📋 Dhammaan'], ['bixiyay','✅ Bixiyay'], ['mabixin','❌ Ma Bixin']].map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)} style={{
            ...
            <span style={{
              ...
              ({val === 'bixiyay' ? bixiyayList.length : val === 'mabixin' ? tiradaMaBixinDhabta : lacagta.length})
            </span>
          </button>
        ))}
      </div>
      ...
      {/* ── ID SEARCH ── */}
      <div style={{ marginBottom:'1.2rem', maxWidth:'280px', position:'relative' }}>
        <input
          value={idSearch}
          onChange={e => setIdSearch(e.target.value)}
          placeholder="🔍 ID ku raadi... (ARD-001)"
          style={{
            width:'100%', padding:'0.6rem 2.2rem 0.6rem 0.9rem',
            border:'2px solid #e8ecf0', borderRadius:'20px',
        ...
        />
        {idSearch && (
          <button onClick={() => setIdSearch('')} style={{
            position:'absolute', right:'10px', top:'50%', transform:'translateY(-50%)',
            background:'none', border:'none', cursor:'pointer', color:'#95a5a6', fontSize:'1rem', fontWeight:'700'
          }}>✕</button>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:'3rem' }}>
            <p style={{ fontSize:'2rem', marginBottom:'0.5rem' }}>
              {idSearch ? '🔍' : filter === 'bixiyay' ? '✅' : filter === 'mabixin' ? '❌' : '📋'}
            </p>
            <p style={{ color:'#95a5a6', fontWeight:'600' }}>
              {idSearch ? `"${idSearch}" ID lama helin` : filter === 'bixiyay' ? 'Lacag bixiyay lama helin' : filter === 'mabixin' ? 'Lacag ma bixin lama helin' : 'Xog lama helin'} — {filterBisha} {filterSannad}
            </p>
          </div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {filteredById.map(l => {
                const isSelectedMonth = isSearching && l.bisha === filterBisha && l.sannadka === filterSannad
                return (
                <tr key={l.id} style={{ borderBottom:'1px solid #f0f4f8', transition:'background 0.1s',
                  background: isSelectedMonth ? '#fffbe6' : 'transparent',
                  outline: isSelectedMonth ? '2px solid #f39c12' : 'none',
                  outlineOffset: '-2px',
                  opacity: l.placeholder ? 0.45 : 1
                }}
                  onMouseEnter={e => e.currentTarget.style.background= isSelectedMonth ? '#fff3cd' : '#f8fafc'}
                  onMouseLeave={e => e.currentTarget.style.background= isSelectedMonth ? '#fffbe6' : 'transparent'}>
                  ...
                  <td style={{ padding:'0.8rem', fontWeight:'700', color: l.xaddiga > 0 ? '#27ae60' : '#95a5a6' }}>
                    {l.placeholder ? '—' : l.xaddiga > 0 ? `$${Number(l.xaddiga).toLocaleString()}` : '—'}
                  </td>
                  <td style={{ padding:'0.8rem' }}>
                    {l.placeholder ? (
                      <span style={{ color:'#bdc3c7', fontSize:'0.78rem' }}>Xog la'aan</span>
                    ) : (
                    <button
