...
export default function Lacagta({ user }) {
  ...
  // Toggle bixiyay/mabixin — labadaba isla-bedel si toos ah
  ...
  // Print Receipt — Printer-friendly, wanaagsan
  const printReceipt = (lacag) => {
    ...
    win.document.write(`
      ...
      <head>
        <meta charset="UTF-8">
        <title>Rasiidh — ${arday?.magaca || 'Arday'}</title>
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .amount-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; opacity: 0.8; }
      ...
      <body>
        <div class="receipt">
          <div class="header">
            ...
            <div class="receipt-title">Rasiidh Lacag — Official Receipt</div>
          </div>
          
          <div class="receipt-num">
            Rasiidh #${Date.now().toString().slice(-6)} &nbsp;|&nbsp; ${taariikhPrint}
          </div>
          ...
          <div class="info-section">
            ...
            <div class="info-row">
              ...
              <span class="info-value">${arday?.telefoon || '—'}</span>
            </div>
            ${lacag.bixiyay ? `<div class="info-row">
          ...
          <div class="status-box ${lacag.bixiyay ? 'status-paid' : 'status-unpaid'}">
            ${lacag.bixiyay ? '✅ LACAGTA WAA LA BIXIYAY' : '❌ LACAGTA MA LA BIXIN'}
          </div>

          <div class="footer">
            <p class="footer-text">Rasiidhan waa heshiis rasmi ah</p>
            <p class="footer-text">Waxaa daabacay: Nidaamka Maamulka Dugsiga</p>
            ...
            <div style="margin-top: 24px; display: flex; justify-content: space-between; padding: 0 10px;">
              <span class="signature-line" style="text-align:center;">Saxeexa Maamulka</span>
              <span class="signature-line" style="text-align:center;">Saxeexa Waalidka</span>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {filtered.map((l) => (
                <tr key={l.id} style={{ borderBottom: '1px solid #f0f4f8' }}
                  ...
                  <td style={{ padding: '0.8rem' }}>
                    {/* Toggle button — admin kaliya, isla bedel */}
                    <button 
                      onClick={() => user.role === 'admin' ? togglePaid(l.id, l.bixiyay) : null}
                      title={user.role === 'admin' ? (l.bixiyay ? 'Guji si aad u baddesho: Ma Bixin' : 'Guji si aad u baddesho: Bixiyay') : ''}
                      style={{
