...
export default function Settings({ user, onUpdateAdmin }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        {[['password','🔑 Password'], ['dugsi','🏫 Magaca Dugsiga']].map(([t, label]) => (
      ...
      {msg && (
        <div style={{ padding: '0.75rem 1rem', background: msgColor==='green'?'#eafaf1':msgColor==='red'?'#fdedec':'#fef9e7', border: '1px solid #ddd', borderRadius: '8px', color: msgColor==='green'?'#27ae60':msgColor==='red'?'#e74c3c':'#f39c12', marginBottom: '1rem', fontWeight: '600' }}>{msg}</div>
      )}
      ...
      {tab === 'password' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', maxWidth: '450px' }}>
          ...
          <div style={{ padding: '0.75rem', background: '#fef9e7', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.82rem', color: '#856404' }}>
      ...
      {tab === 'dugsi' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', maxWidth: '450px' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <input value={dugsiMagac} onChange={e => setDugsiMagac(e.target.value)} placeholder="Tusaale: Dugsi Hoose Xoriyo"
              style={{ width: '100%', padding: '0.75rem', border: '2px solid #e8ecf0', borderRadius: '8px', fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box' }} />
          </div>
