...
// ============================================================
// PASSWORD FIELD - DEFINED OUTSIDE PasswordChange
// FIX: Component-ka waa la qoray DIBADDA PasswordChange si
//      dib u abuurista (re-render) focus-ka uusan ka luminin
// ============================================================
...
function PasswordChange() {
  ...
  // Waa la qoray si stable ah - ma keeni doono re-render
  ...
  function handleSubmit() {
    ...
    if (form.new_ !== form.confirm){ setMsg("❌ Password-yada cusub kuma xidna."); return; }

    const current = getCurrentPassword();
    if (form.old !== current){ setMsg("❌ Password-kii hore waa khalad.");        return; }
    if (form.new_ === current){ setMsg("❌ Password cusub isku mid baa ahan kan hore."); return; }

    // Keyd admin_credentials - isku mid Login.js
    let email = "admin@dugsi.so";
    try {
      const stored = localStorage.getItem("admin_credentials");
      if (stored) email = JSON.parse(stored).email || email;
    } catch {}
  ...
  return (
    <div>
      <h3 style={{ margin: "0 0 20px", fontSize: 16, fontWeight: 600, color: "#1a1a1a" }}>
        🔑 Password Beddel
      </h3>

      {/* FIX: autoComplete="off" form-ka oo dhan si browser uusan buuxin */}
      <div autoComplete="off">
        {/* Hidden honeypot inputs - trick browser autocomplete */}
        <input type="text"     style={{ display: "none" }} readOnly />
        <input type="password" style={{ display: "none" }} readOnly />

        <PasswordField
          label="Password-kii Hore"
          fieldKey="old"
          value={form.old}
          showPw={show.old}
...
function ColorCustomizer({ settings, onSave }) {
  ...
  return (
    <div>
      ...
      <button type="button" onClick={handleSave} style={{ width: "100%", padding: "11px", fontWeight: 700, fontSize: 15, background: saved ? "#d1fae5" : local.accentColor, color: saved ? "#065f46" : "#fff", border: "none", borderRadius: 9, cursor: "pointer", transition: "all 0.3s" }}>
