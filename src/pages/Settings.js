...
export default function Settings({ user }) {
  ...
  const changePassword = () => {
    ...
    // Always read fresh from localStorage at the moment of checking
    let saved
    try {
    ...
    // Compare with both saved password AND default (in case localStorage was cleared)
    const correctPassword = saved.password || defaultCreds.password
    if (oldPass !== correctPassword) return showMsg('❌ Password-kii hore khalad! Haddii aadan wax badbalin isku day: Admin@2024', 'red')
      ...
      {tab === 'password' && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', maxWidth: '450px' }}>
          ...
          <div style={{ padding: '0.75rem', background: '#fef9e7', borderRadius: '8px', marginBottom: '1.2rem', fontSize: '0.82rem', color: '#856404' }}>
            ⚠️ Password-ka cusub ugu yaraan 6 xaraf ahaan. Password-ka default waa: <strong>Admin@2024</strong>
          </div>
          <button onClick={changePassword} style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg, #1a3a5c, #3498db)', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '700', fontSize: '1rem', marginBottom: '0.75rem' }}>
            🔑 Password Beddel
          </button>
          <button onClick={() => {
            const defaultCreds = { email: 'admin@dugsi.so', password: 'Admin@2024' }
            localStorage.setItem('admin_credentials', JSON.stringify(defaultCreds))
            setOldPass(''); setNewPass(''); setNewPass2('')
            showMsg('✅ Password-ka waxaa loo celiyay Admin@2024', 'green')
          }} style={{ width: '100%', padding: '0.7rem', background: '#fff', color: '#e74c3c', border: '2px solid #e74c3c', borderRadius: '10px', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem' }}>
            🔄 Password Default-ka Celi (Admin@2024)
          </button>
        </div>
      )}
