...
export default function Ardayda() {
  ...
  const printIdCard = (a) => {
    ...
    // ── Soo qaad school_settings - mar kasta marka la furo (no cache) ──
    ...
    try {
      ...
      if (raw) {
        ...
        if (ss.schoolName && ss.schoolName.trim()) schoolName = ss.schoolName.trim()
        if (ss.logoUrl)    logoUrl    = ss.logoUrl
        if (ss.accentColor) accentColor = ss.accentColor
        if (ss.sidebarColor) accentColor = ss.sidebarColor
      }
    } catch {}
    // Fallback chain
    if (schoolName === 'Nidaamka Dugsiga') {
      const fallback = localStorage.getItem('dugsi_magac')
      if (fallback) schoolName = fallback
    }
