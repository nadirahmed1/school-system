...
const empty = { magaca: '', hooyo_magac: '', telefoon: '', fasalka: 'Fasal 1', arday_id_gaar: '', taariikhda_diiwaangelinta: new Date().toISOString().split('T')[0] }
...
export default function Ardayda() {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const idGaar = form.arday_id_gaar || generateId(ardayda)
      const saveData = { ...form, arday_id_gaar: idGaar }
      if (editId) {
        await supabase.from('ardayda').update(saveData).eq('id', editId)
        setMsg('✅ Waa la beddelay!')
      } else {
        await supabase.from('ardayda').insert([saveData])
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
      ...
      reader.onload = async (evt) => {
        ...
        const existing = ardayda.length
        const ardaydaNew = rows.map((row, i) => ({
          magaca: row['Magaca'] || row['magaca'] || '',
          hooyo_magac: row['Magaca Hooyada'] || row['hooyo_magac'] || '',
          telefoon: String(row['Telefoon'] || row['telefoon'] || ''),
          fasalka: row['Fasalka'] || row['fasalka'] || 'Fasal 1',
          arday_id_gaar: row['ID'] || `ARD-${(existing + i + 1).toString().padStart(3,'0')}`,
          taariikhda_diiwaangelinta: new Date().toISOString().split('T')[0]
        })).filter(a => a.magaca)

        if (ardaydaNew.length === 0) {
          setMsg('❌ Excel-ka xog lama helin! Tiirarka hubi.')
          return
        }
        const { error } = await supabase.from('ardayda').insert(ardaydaNew)
        if (error) setMsg('❌ Upload khalad: ' + error.message)
        else { setMsg(`✅ ${ardaydaNew.length} arday la daray!`); fetchArdayda() }
        setTimeout(() => setMsg(''), 4000)
      }
      reader.readAsBinaryString(file)
    } catch(e) { setMsg('❌ Khalad!') }
    e.target.value = ''
  }
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '480px', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {[
              { label: 'ID Arday', key: 'arday_id_gaar', placeholder: 'ARD-001' },
