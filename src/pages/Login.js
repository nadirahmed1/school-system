...
// ============================================================
// Hubi - haddii admin_credentials localStorage-ka aan jirin,
// keyd default-ka si Settings-ku u helo marka hore
// ============================================================
function ensureAdminCredsExist() {
  try {
    const raw = localStorage.getItem('admin_credentials')
    if (!raw) {
      localStorage.setItem('admin_credentials', JSON.stringify(DEFAULT_ADMIN))
    } else {
      // Hubi inuu valid JSON yahay
...
export default function Login({ onLogin }) {
  ...
  // Marka component-ku billaabo, xaqiiji credentials jiraan
  useEffect(() => {
    ensureAdminCredsExist()
  }, [])
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Check admin - ka soo jiid localStorage (ama default)
      const savedAdmin = JSON.parse(
        localStorage.getItem('admin_credentials') || JSON.stringify(DEFAULT_ADMIN)
      )
      ...
      if (email === savedAdmin.email && password === savedAdmin.password) {
        onLogin({
          ...
          role: 'admin',
          email,
          dugsi_magac: localStorage.getItem('dugsi_magac') || 'Nidaamka Dugsiga'
        })
        return
      }
      ...
      onLogin({
        ...
        email,
        id: data.id,
        dugsi_magac: localStorage.getItem('dugsi_magac') || 'Dugsigeena'
      })
    } catch (err) {
