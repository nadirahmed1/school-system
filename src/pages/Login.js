  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (email === savedAdmin.email && password === savedAdmin.password) {
        onLogin({ magaca: 'Admin', role: 'admin', email, dugsi_magac: localStorage.getItem('dugsi_magac') || 'Dugsigeena' })
        return
      }
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '420px', padding: '0 1.5rem', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#f0f7ff', borderRadius: '8px', fontSize: '0.78rem', color: '#5d6d7e' }}>
            <strong>Admin:</strong> admin@dugsi.so / Admin@2024
          </div>
        </div>
      </div>
    </div>
